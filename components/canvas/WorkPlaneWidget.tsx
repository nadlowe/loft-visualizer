...
import { WorkPlane } from "./render/renderWorkPlane";
...
export function WorkPlaneWidget({
  ...
  const translationGroupRef = useRef<THREE.Group | null>(null);
  const [translationGroup, setTranslationGroup] = useState<THREE.Group | null>(
    null
  );
  ...
  const rotationControlsAnchorRef = useRef<THREE.Group | null>(null);
  const [rotationControlsAnchor, setRotationControlsAnchor] =
  ...
  const setTranslationGroupRef = (el: THREE.Group | null) => {
    translationGroupRef.current = el;
    setTranslationGroup(el);
  };

  const setRotationControlsAnchorRef = (el: THREE.Group | null) => {
    rotationControlsAnchorRef.current = el;
    setRotationControlsAnchor(el);
  };
  ...
  const { camera, size: frameSize } = useThree();
  const [helperScale, setHelperScale] = useState(1);

  // Update helper scale for consistent screen size
  useFrame(() => {
    ...
    if ((camera as THREE.OrthographicCamera).isOrthographicCamera) {
      ...
      // Use distance to workPlane origin for perspective scaling
      const worldPos = new THREE.Vector3().copy(workPlane.position);
    ...
    // Target ~80 pixels for the main helper axis
    const newScale = 80 * worldUnitsPerPixel;
    if (Math.abs(helperScale - newScale) > newScale * 0.01) {
      setHelperScale(newScale);
    }
  });

  // Force TransformControls update on mount and camera change
  useEffect(() => {
    const timer = setTimeout(() => {
      if (translateControlsRef.current) translateControlsRef.current.update();
      if (rotateControlsRef.current) rotateControlsRef.current.update();
    }, 0);
  ...
  return (
    <group>
      ...
      <group
        ref={setTranslationGroupRef}
        position={workPlane.position}
        rotation={[0, 0, 0]}
      >
        {/* Rotation group - contains the visual representation */}
        <group ref={rotationGroupRef} rotation={workPlane.rotation}>
          ...
          {/* Helpers - shown in plane's local space, scaled for screen space visibility */}
          {showHelpers && (
            <group scale={helperScale}>
        ...
        {/* Empty group at origin for rotation controls positioning */}
        <group
          ref={setRotationControlsAnchorRef}
          position={[0, 0, 0]}
          rotation={workPlane.rotation}
        />
      </group>
      ...
      {enabled && showRotate && isShiftHeld && rotationControlsAnchor && (
        <TransformControls
          ref={rotateControlsRef}
          object={rotationControlsAnchor}
          mode="rotate"
          space="local"
          size={0.5}
      ...
      {enabled && showTranslate && !isShiftHeld && translationGroup && (
        <TransformControls
