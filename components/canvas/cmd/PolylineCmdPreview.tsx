...
export function PolylineCmdPreview({
  ...
  const closeLineRef = useRef<Line2>(null);
  ...
  useFrame(({ camera, size: frameSize }) => {
    ...
    const closeLineObj = closeLineRef.current;
    const snapIndicatorObj = snapIndicatorRef.current;

    // Update close loop dashed line
    if (closeLoop && hoverPosition && vertices.length >= 2 && closeLineObj) {
      ...
      closeLineObj.geometry.setPositions(positions);
      closeLineObj.computeLineDistances();
      closeLineObj.visible = true;

      // Update dash size for screen-space scaling
      const material = closeLineObj.material as LineMaterial;
      material.dashSize = 8 * worldUnitsPerPixel;
      material.gapSize = 4 * worldUnitsPerPixel;
    } else if (closeLineObj) {
      closeLineObj.visible = false;
    }
    ...
    if (
      ...
      snapIndicatorObj.visible = true;
      // Set position with small z offset to render above ground plane
      snapIndicatorObj.position.set(snapPosition.x, snapPosition.y, 0.01);
      // ~10 pixel crosshair (scale from -1 to 1 geometry = 2 units, so *0.5 for radius)
      const indicatorSize = 10 * worldUnitsPerPixel;
      snapIndicatorObj.scale.set(indicatorSize, indicatorSize, 1);
    } else if (snapIndicatorObj) {
      snapIndicatorObj.visible = false;
    }
  });
  ...
  return (
    <>
      ...
      <primitive ref={closeLineRef} object={closeLine} />
      <primitive ref={snapIndicatorRef} object={snapIndicator} />
    </>
  );
}
