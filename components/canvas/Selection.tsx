...
export function Selection({
  ...
  const { raycaster, camera, gl: renderer, scene } = useThree();
  const {
    selectOnly,
    toggleSelection,
    selectMultiple,
  ...
  const spaceKeyPressedRef = useSpaceKeyPressed(renderer.domElement);
  ...
  const [prevEditingPolylineId, setPrevEditingPolylineId] =
    useState(editingPolylineId);
  if (editingPolylineId !== prevEditingPolylineId) {
    setPrevEditingPolylineId(editingPolylineId);
    if (editingPolylineId && selectionRect) {
  ...
  const handleWindowSelection = useWindowSelection(
    scene,
    selectMultiple,
    selectedHandles,
    clearSelection
  ...
  const handleClickSelection = useClickSelection({
    scene,
    is2D,
    camera,
    renderer,
...
function useSpaceKeyPressed(domElement: HTMLCanvasElement) {
