...
export function PolylineSelection({ is2D }: PolylineSelectionProps) {
    ...
    const handleMouseUp = (e: MouseEvent) => {
      ...
      if (is2D && selectionRect && moved) {
        ...
        if (startWorld && endWorld) {
          ...
          const polylines: Line2[] = [];
          scene.traverse((object) => {
            if (object instanceof Line2 && object.userData.handle) {
          ...
          if (selectedLines.length > 0) {
            const handles = selectedLines.map(
              (line) => line.userData.handle as EntityHandle
            );
      ...
      if (closest) {
        e.preventDefault();
        e.stopPropagation();
        const handle = closest.line.userData.handle as EntityHandle;
