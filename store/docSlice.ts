import { defaultDocInit } from "@/lib/doc/defaultDoc";
...
} from "@/lib/entity/entityTools/entityDuplicate";
import { entityTypeToDocField } from "@/lib/entity/entityTools/entityTypeToDocField";
import { EntityTypeToEntity } from "@/lib/entity/entityTools/entityTypeToEntity";
import { handleNew } from "@/lib/entity/handleTools/handleNew";
import { parseHandle } from "@/lib/entity/handleTools/handleTools";
import { EntityHandle, SelectableHandle } from "@/lib/entity/handleTypes";
import { adjustLoftSeamsAfterPolylineEdit } from "@/lib/geom/utils";
import { EntityId, PolylineId } from "@/lib/util/uid";
  ...
  return {
    ...
    deleteEntity: (handle) => {
      ...
      transact((state) => {
        ...
        const { [id]: _removed, ...rest } = currentTable;
        return {
          doc: {
            ...state.doc,
            [fieldName]: rest,
    ...
    setHidden: (handles, hidden) => {
      transact((state) => {
        ...
        for (const handle of handles) {
          ...
          const fieldName = entityTypeToDocField[type];
          const entity = (
            newDoc[fieldName] as Record<string, BaseEntity<EntityId>>
          )[id];
          if (!entity) continue;
